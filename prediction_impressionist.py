...
df = pd.read_csv("data/data_impressionist.csv")
"""
...
def show_input_and_palettes(inputs, predictions):
    ...
    for i, (input_lab, pred) in enumerate(zip(inputs, predictions)):
        input_temp = (
            np.clip(np.array(input_lab) * 255, 0, 255).astype(int).reshape(1, 1, 3)
        )
        ...
        axes[i].imshow(
            cv2.cvtColor(np.array(row_colors, dtype=np.uint8), cv2.COLOR_LAB2RGB)
        )
        axes[i].set_xticks([])
        axes[i].set_yticks([])
...
model.save("data/impressionist_paintings3.keras")
